...
void import_galaxies() {
	...
	if (file != NULL) {
		...
		//while(!feof(file)) {
		while(fgets(line, sizeof(line), file) != NULL && nb_galaxies < 2000000) {

			fscanf(file, "%lf %lf %lf", &x, &y, &z);
			Galaxies[nb_galaxies].x = (double)(30856775814672000.0 * 1000000.0 * x) - 5*1e24;
			Galaxies[nb_galaxies].y = (double)(30856775814672000.0 * 1000000.0 * y) - 5*1e24;
			Galaxies[nb_galaxies].z = (double)(30856775814672000.0 * 1000000.0 * z) - 5*1e24;
...
void importStarsDB() {
	...
	if (file != NULL) {
		...
		while(fgets(line, sizeof(line), file) != NULL) {
			...
			Stars[nb_stars].id = atoi(temp_id);
			Stars[nb_stars].x = atof(temp_dist)*300000000.0*3600.0*24.0*365.0*cos(M_PI*atof(temp_glon)/180)*cos(M_PI*atof(temp_glat)/180);
			Stars[nb_stars].y = atof(temp_dist)*300000000.0*3600.0*24.0*365.0*sin(M_PI*atof(temp_glon)/180)*cos(M_PI*atof(temp_glat)/180);
			Stars[nb_stars].z = atof(temp_dist)*300000000.0*3600.0*24.0*365.0*sin(M_PI*atof(temp_glat)/180);
